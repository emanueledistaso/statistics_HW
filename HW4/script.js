     function generateColor() {
      ...
      function drawGraph(paths, n) {
        ...
        let ystep = maxValue > 5 ? height / (maxValue * 2.4) : height / 10;
        maxValue = Math.max(Math.max(...paths.flat()));
        let stepLineBox = document.getElementById(
          "step_line_checkmark"
        ).checked;
      ...
      function runSimulation(n, lambda, pathNum, time) {
        ...
        document.getElementById(
          "avg_display"
        ).innerHTML = `<p>Mean: ${mean.toFixed(4)}</p>
  ...
  <p>Distance between total relative success frequency and probability: ${(
    ...
    lambda / n
      ...
      document.getElementById("my_btn").addEventListener("click", () => {
        ...
        runSimulation(n, lambda, pathNum, time);
      });
